...
from app.models import Job, Freelancer, User
...
def get_freelancers():
    ...
    return jsonify([{'id': f.id, 'name': f.name, 'email': f.email, 'skills': f.skills} for f in freelancers])
...
def get_freelancers():
    freelancers = Freelancer.query.all()
    return jsonify([{'id': freelancer.id, 'name': freelancer.name} for freelancer in freelancers])
...
def update_freelancer(id):
    ...
    if freelancer.user_id != current_user.id:
        return jsonify({'message': 'You are not authorized to update this freelancer profile'}), 403
...
def delete_freelancer(id):
    freelancer = Freelancer.query.get_or_404(id)
    if freelancer.user_id != current_user.id:
        return jsonify({'message': 'You are not authorized to delete this freelancer profile'}), 403
